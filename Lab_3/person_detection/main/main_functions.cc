...
#define TFLITE_USE_EXTERNAL_ALLOCATOR
#define THRESHOLD 0
...
void setup() {
  ...
  model = tflite::GetModel(g_mnist_digits_tiny_int8_tflite);
...
void loop() {
  ...
  if (max_score < THRESHOLD) {
    MicroPrintf("Nada detectado (confianza baja)");
  } else {
    MicroPrintf("Clase %d → %d", predicted, max_score);
  }

  for (int i = 0; i < 10; ++i) {
    MicroPrintf("Clase %d → %d", i, input->data.int8[i]);
  }
